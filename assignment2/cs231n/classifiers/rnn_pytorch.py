...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        h0 = affine_forward(features, W_proj, b_proj)  # 1
        x = word_embedding_forward(captions_in, W_embed)  # 2
        h = None
        if self.cell_type == "rnn":
            h = rnn_forward(x, h0, Wx, Wh, b)  # 3
        scores = temporal_affine_forward(h, W_vocab, b_vocab)  # 4
        loss = temporal_softmax_loss(scores, captions_out, mask)  # 5
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def sample(self, features, max_length=30):
        ...
        for i in range(max_length):
            prev_word_embed = word_embedding_forward(prev_word, W_embed)  # 1
            prev_h = rnn_step_forward(prev_word_embed, prev_h, Wx, Wh, b) # 2
            scores = affine_forward(prev_h, W_vocab, b_vocab)
            prev_word = torch.argmax(scores, axis = 1, keepdims=True) # 3
            captions[:, i] = prev_word.squeeze(1) # 4
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return captions
