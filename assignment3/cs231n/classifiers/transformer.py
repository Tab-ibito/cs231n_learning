...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        tgt_mask = torch.tril(tgt_mask) # 2

        output = self.transformer(captions, features, tgt_mask)
        scores = self.output(output) # 3
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.positional_encoding(x) # 2
        x = self.transformer(x) # 3
        x = torch.mean(x, dim=1) # 4
        logits = self.head(x) # 5
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
