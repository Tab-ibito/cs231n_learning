...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    pred_classes = [class_texts[elem] for elem in torch.argmax(similarity, dim=0)]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        self.images_tensor = torch.cat(self.processed_images, dim=0).to(device)
        self.image_features = clip_model.encode_image(self.images_tensor)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        pass
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        text_feature = self.clip_model.encode_text(token)
        similarity = get_similarity_no_loop(text_feature, self.image_features)
        top_indices = [elem for elem in torch.topk(similarity, k, dim=1, largest=True, sorted=True).indices.view(-1)]
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return top_indices
...
class DINOSegmentation:
    def __init__(self, device, num_classes: int, inp_dim: int = 384):
        ...
        ############################################################################
        self.ln = nn.Linear(inp_dim, num_classes, device=device)
        self.optim = torch.optim.Adam(self.ln.parameters(), lr=0.001)
    ...
    def inference(self, X_test):
        ...
        ############################################################################
        self.ln.eval()
        pred_classes = torch.argmax(self.ln(X_test), dim=1)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return pred_classes