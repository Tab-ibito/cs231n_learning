...
class GaussianDiffusion(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        ...
        ####################################################################
        coef_x = extract(self.sqrt_alphas_cumprod, t, x_t.shape)
        coef_noise = extract(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape)
        x_start = (x_t - coef_noise * noise) / coef_x
        ####################################################################
        return x_start
    ...
    def predict_noise_from_start(self, x_t, t, x_start):
        ...
        # Look at the coeffs in `__init__` method and use the `extract` function.
        ####################################################################
        coef_x = extract(self.sqrt_alphas_cumprod, t, x_start.shape)
        coef_noise = extract(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape)
        pred_noise = (x_t - coef_x * x_start) / coef_noise
        ####################################################################
        return pred_noise
    ...
    def p_sample(self, x_t, t: int, model_kwargs={}):
        ...
        #      and sample x_{t-1}.
        ##################################################################
        model_pred = self.model(x=x_t, time=t, model_kwargs=model_kwargs)

        if self.objective == "pred_noise":
            x_start = self.predict_start_from_noise(x_t, t, model_pred)
        else:
            x_start = model_pred
    ...
    def q_sample(self, x_start, t, noise):
        ...
        coef_x = extract(self.sqrt_alphas_cumprod, t, x_start.shape)
        coef_noise = extract(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape)
        x_t = coef_x * x_start + coef_noise * noise
        ####################################################################
        return x_t
    ...
    def p_losses(self, x_start, model_kwargs={}):
        ...
        model_pred = self.model(x=x_t, time=t, model_kwargs=model_kwargs)
        loss = torch.mean(loss_weight * (model_pred - target) ** 2)
        ####################################################################
