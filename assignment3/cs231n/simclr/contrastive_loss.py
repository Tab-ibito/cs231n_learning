...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        ##############################################################################
        out_sim = torch.zeros(2 * N)
        for i in range(2 * N):
            z_i = out[i]
            out_sim[i] = sim(z_k, z_i) / tau
        out_sim_exp = torch.exp(out_sim)
        mask = torch.ones(2 * N, dtype=torch.bool)
        mask[k] = 0
        l1 = - torch.log(out_sim_exp[k + N] / torch.sum(out_sim_exp[mask]))

        for i in range(2 * N):
            z_i = out[i]
            out_sim[i] = sim(z_k_N, z_i) / tau
        out_sim_exp = torch.exp(out_sim)
        mask = torch.ones(2 * N, dtype=torch.bool)
        mask[k + N] = 0
        l2 = - torch.log(out_sim_exp[k] / torch.sum(out_sim_exp[mask]))
...
def sim_positive_pairs(out_left, out_right):
    ...
    # HINT: torch.linalg.norm might be helpful.                                  #
    ##############################################################################
    pos_pairs = torch.diag(
        torch.matmul(out_left / torch.linalg.norm(out_left, dim=1, keepdim=True),
                     (out_right / torch.linalg.norm(out_right, dim=1, keepdim=True)).t())
    )[:, None]
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    similarity = sim_positive_pairs(out_left, out_right).repeat(2, 1)
