...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        scale = 10000 ** (-torch.arange(0, embed_dim, 2) / embed_dim)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query_multi = self.query(query)
        key_multi = self.key(key)
        value_multi = self.value(value)

        query_multi = query_multi.reshape(N, S, H, head_dim).transpose(1, 2)
        key_multi = key_multi.reshape(N, T, H, head_dim).transpose(1, 2)
        value_multi = value_multi.reshape(N, T, H, head_dim).transpose(1, 2)
        E_multi = torch.matmul(query_multi, torch.transpose(key_multi, 2, 3)) / math.sqrt(head_dim)  # shape N, H, S, T
        if attn_mask is not None:
            E_multi = E_multi.masked_fill(attn_mask[None, None, :, :] == 0, torch.finfo(E_multi.dtype).min)
        A = F.softmax(E_multi, dim=3)
        A = self.attn_drop(A)
        output = torch.matmul(A, value_multi).transpose(1, 2).contiguous().reshape(N, S, E)
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.reshape(x, (N, C, grid_size, patch_size, grid_size, patch_size))
        x = torch.permute(x, (0, 2, 4, 1, 3, 5))
        x = torch.reshape(x, (N, num_patches, -1))
        out = self.proj(x)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
