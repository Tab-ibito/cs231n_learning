...
class Unet(nn.Module):
    def __init__(
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            ##################################################################
            down_block = nn.ModuleList([ResnetBlock(dim_in, dim_in, context_dim), ResnetBlock(dim_in, dim_in, context_dim), Downsample(dim_in, dim_out)])
            ##################################################################
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out_ups):
            ...
            ##################################################################
            up_block = nn.ModuleList([Upsample(dim_in, dim_out), ResnetBlock(2*dim_out, dim_out, context_dim), ResnetBlock(2*dim_out, dim_out, context_dim)])
    ...
    def cfg_forward(self, x, time, model_kwargs={}):
        ...
        empty["text_emb"] = None
        x = (cfg_scale + 1) * self.forward(x, time, model_kwargs) - cfg_scale * self.forward(x, time, empty)
        ##################################################################
